...
export default function PasswordGate({ children, lang }: Props) {
  ...
  // Always render children — overlay sits on top with fixed positioning
  // This avoids SSR hydration mismatch from conditional children mounting
  return (
    <>
      {children}
      ...
      {(!mounted || !unlocked) && (
        <div className="fixed inset-0 z-[9999] bg-noir flex items-center justify-center px-4">
          <div className="w-full max-w-md">
            ...
            {/* Form only shown after hydration to avoid SSR mismatch */}
            {mounted && (
